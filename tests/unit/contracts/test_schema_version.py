...
def make_capture_record() -> CaptureRecord:
    """A minimal valid capture record."""
...
def test_current_schema_version_is_supported() -> None:
    ...
    assert frozenset({"0.1"}) == SUPPORTED_SCHEMA_VERSIONS
...
@pytest.mark.parametrize("version", ["0.2", "1.0", "0.1.0", "", "latest"])
...
def test_unknown_schema_version_is_rejected_on_envelopes() -> None:
    ...
    with pytest.raises(ValidationError, match="schema_version"):
        CaptureEnvelope.model_validate(data | {"schema_version": "0.2"})
