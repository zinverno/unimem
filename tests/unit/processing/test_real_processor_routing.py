"""The five real processors in one router, and the routing that follows.

The router's exactly-one-match rule was a formality with one processor and real
work with two. With five — two of them claiming *MIME types* within the same
``DOCUMENT`` payload type, and one claiming two MIME types within ``IMAGE`` — it
is what keeps formats apart from each other, and Phase 3 PR 2 is where that
stopped being hypothetical: a PDF capture and a DOCX capture are the same payload
type, and only the declared format tells them apart. Phase 4 PR 1 adds the case
from the other direction — one processor claiming two formats of one payload type
— and the rule needs no adjusting for it. These tests use the genuine processor
set the composition root registers, in the order it registers them, and assert
that order never decides anything.
"""
...
from tests import docxs, images, pdfs
...
def png_capture(store: InMemoryRawObjectStore, **overrides: object) -> CaptureRecord:
    fields: dict[str, object] = {"payload_type": CapturePayloadType.IMAGE}
    return store_and_capture(store, images.png(), mime_type=PNG_MIME_TYPE, **(fields | overrides))


def jpeg_capture(store: InMemoryRawObjectStore, **overrides: object) -> CaptureRecord:
    fields: dict[str, object] = {"payload_type": CapturePayloadType.IMAGE}
    return store_and_capture(store, images.jpeg(), mime_type=JPEG_MIME_TYPE, **(fields | overrides))
...
@pytest.mark.parametrize("build", [png_capture, jpeg_capture], ids=["png", "jpeg"])
def test_an_image_capture_reaches_the_image_processor(
    router: ProcessorRouter,
    processors: list[Processor],
    store: InMemoryRawObjectStore,
    build: Callable[[InMemoryRawObjectStore], CaptureRecord],
) -> None:
    """One processor, two formats — the mirror image of the document case."""
    assert router.select(build(store)) is processors[4]


def test_the_image_processor_claims_no_document(
    processors: list[Processor], store: InMemoryRawObjectStore
) -> None:
    image = processors[4]

    assert not image.supports(pdf_capture(store))
    assert not image.supports(docx_capture(store))


def test_the_document_processors_claim_no_image(
    processors: list[Processor], store: InMemoryRawObjectStore
) -> None:
    pdf, docx = processors[2], processors[3]

    assert not pdf.supports(png_capture(store))
    assert not docx.supports(png_capture(store))


def test_a_deferred_image_format_reaches_nobody(
    router: ProcessorRouter, store: InMemoryRawObjectStore
) -> None:
    """An ``image/webp`` capture is a loud routing error rather than a guess."""
...
def test_a_second_image_processor_is_an_error_too(
    processors: list[Processor], store: InMemoryRawObjectStore
) -> None:
    router = ProcessorRouter([*processors, ImageProcessor(store)])

    with pytest.raises(AmbiguousProcessorError):
        router.select(png_capture(store))
...
@pytest.mark.parametrize(
    "build",
    [pdf_capture, docx_capture, png_capture, jpeg_capture],
