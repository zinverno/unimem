...
> **Macro Phase 4 ОТКРЫТА.** Приёмка владельца уже проводилась — по исходному
> критерию IMG-E; она дала **4 из 5** и записана в
> [истории прогонов](#история-прогонов-владельца). После уточнения критерия
> нужен новый полный прогон, и его ещё не было: все пять строк
> [таблицы результатов](#таблица-результатов) стоят в `NOT_RUN`. Закрытие фазы —
> отдельное изменение, которое делается **после** этого нового прогона и только
> по его результату.
...
Заполняется **владельцем**. **Новый закрывающий прогон ещё не выполнен** — тот,
что выше, проводился по прежней, более широкой формулировке IMG-E и для
закрытия не годится.
...
- **Окружение:** — (дистрибутив, Python, версия Tesseract, наличие `eng` и
  `rus`)
- **Итог:** `0 из 5` — закрывающий прогон не выполнялся.
...
Это таблица **закрывающего** прогона. Результаты прогона 2026-09-17 сюда **не
переносятся** — ни одной строкой, включая четыре его `PASS`.
...
| IMG-E | NOT_RUN | — | одно настоящее изображение владельца с обычным печатным текстом |
...
3. **Это изменение** — уточнение области действия приёмки после того прогона.
   Оно сужает IMG-E до обычного печатного текста, вносит прогон 2026-09-17 в
   архив как состоявшуюся приёмку и не трогает ни продукт, ни
   `docs/ARCHITECTURE.md`, ни ADR. Macro Phase 4 после него по-прежнему
   **ОТКРЫТА**.
4. Владелец проводит **новый полный прогон** против слитого `main`, содержащего
   это уточнение. Тот SHA и становится базовой версией закрывающей приёмки.
5. **Закрывающее изменение** — тоже только документация — записывает результат
   и закрывает Macro Phase 4 в `docs/ARCHITECTURE.md`, **если и только если**
   все пять исправленных строк `PASS`.

Шаг 3 не изобретает новую фазу: это уточнение критерия приёмки внутри Phase 4C,
а не ещё один срез реализации. **Никакой Phase 4D нет и не предполагается.**
