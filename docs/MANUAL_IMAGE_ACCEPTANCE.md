...
> **Macro Phase 4 ЗАКРЫТА приёмкой владельца 2026-09-18.** Закрывающий прогон
> выполнен вручную против слитого `main`
> `f436492ce59cecbbce4ebed321ad7e4cf6326be8` и дал **5 из 5** — см.
> [таблицу результатов](#таблица-результатов). Более ранняя приёмка 2026-09-17
> по исходному критерию IMG-E дала **4 из 5** и фазу не закрыла; она сохранена
> в [истории прогонов](#история-прогонов-владельца) как есть, и ни один её
> результат в закрывающий прогон не переносился.
...
после того, как изменение с **исправленным критерием IMG-E** будет слито в
`main`.

Порядок такой:

1. изменение с исправленным критерием сливается в `main`;
...
> **Закрывающий прогон выполняется целиком заново.** Это новый каталог
> `image-run-<метка времени>` из шага 1, новые capture id и все пять строк
> IMG-A…IMG-E подряд. Ни одна строка прошлого прогона — включая четыре его
> `PASS` — в новую таблицу не переносится.
...
### Какое изображение годится для этой строки

Возьмите **одно** настоящее изображение PNG или JPEG с **обычным печатным
(наборным) текстом**, который вы сами отчётливо читаете.

Это условие не о том, **что продукт принимает**: принимает он PNG и JPEG широко,
и распознавание на поддержанном изображении в пределах бюджета запускается в
любом случае. Условие о том, **какое качество распознавания закрытие фазы
обязано показать**. Требовать его честно можно от фиксированной политики
Phase 4B — `eng+rus`, OEM 1, PSM 3, без коррекции ориентации, без deskew, без
повторов — именно на обычном печатном тексте. Поэтому таким и должен быть
обязательный образец.

Признаки годного образца:

- обычные печатные (наборные) начертания;
- привычная ориентация текста;
- **без заметных перспективных искажений**;
- **без существенных перекрытий текста**;
- текст отчётливо читается человеком;
- разумный контраст;
- чтобы прочитать буквы, не нужно преодолевать намеренные декоративные
  искажения;
- формат PNG или JPEG;
- язык — **английский или русский**, на ваш выбор.

**Чистая фотография годится**, если отвечает этим условиям. Строка не сводится
к снимкам экрана и не заменяется синтетикой: это по-прежнему **одно настоящее
изображение владельца** в PNG или JPEG.

Годиться может, например: обычный снимок экрана с текстом интерфейса или
документа; чистая фотография обычного печатного текста; чистый скан или снимок
печатной страницы, этикетки, объявления или вывески; любое другое ваше
изображение с обычным печатным текстом. Один конкретный источник здесь не
предписывается.

> **Какой язык вы выбрали — сообщать не нужно.** Ни в этом файле, ни в
> отправляемых доказательствах язык вашего изображения не фиксируется.

### Что НЕ требуется для закрытия Macro Phase 4

Обязательный образец IMG-E **не** обязан проверять качество распознавания на:

- рукописном тексте;
- начертаниях «под рукопись»;
- каллиграфии;
- логотипах и словесных знаках;
- декоративных акцидентных начертаниях;
- сильно стилизованной типографике;
- изогнутом или заметно деформированном тексте;
- тексте в кадре с сильной перспективой;
- существенно перекрытом тексте;
- тексте с очень низким контрастом;
- общем понимании изображения (vision).

Это области качества распознавания и возможного развития. Они **не** делают
такие изображения неподдержанным входом и **не** делают их негодным
содержимым. Разница важная, и её стоит держать раздельно:

- **Вход продукта:** PNG и JPEG поддержаны, такие изображения принимаются и
  хранятся как обычно.
- **Попытка распознавания:** сборка с `--image-ocr` запускает движок на них
  так же, как на любом другом поддержанном изображении в пределах бюджета.
  Распознавание на них **не** отключается.
- **Требование к закрытию:** приемлемое человеком качество распознавания
  обязательно только на образце в области действия — с обычным печатным
  текстом. Для перечисленных выше классов закрытие Macro Phase 4 качества
  **не обещает**.

Ничего нового в продукт это не добавляет: ни формата, ни поля запроса, ни
режима, ни ключа, ни запасного пути, ни движка, ни классификатора.
...
| | условие |
| --- | --- |
| 0 | образец — в области действия: обычный печатный текст (см. выше) |
| 1 | capture — `201` |
...
| 12 | вы считаете распознанное приемлемым по сравнению с тем, что читаете на изображении |

Технические условия 1–11 не ослаблены и ослаблению не подлежат. Условие 0 не
делает их мягче — оно лишь говорит, **на каком образце** эта строка вообще
что-то значит.

**`FAIL`, если** образец в области действия (условие 0), движок отработал
(`engine_invoked = true`), но не выполнено любое из условий 1–3, 5–11 или
условие 12. В частности: движок отработал, а непробельного `ocr`-сегмента нет,
**хотя печатный текст на изображении вы читаете глазами**, — это `FAIL`, а не
«так вышло». Точно так же `FAIL` — если распознанное вы считаете неприемлемым
для обычного печатного образца.

**Если образец вне области действия** (рукопись, декоративное начертание,
логотип, текст в перспективе и прочее из списка выше), продукт отработает как
обычно — изображение примется, сохранится, движок на нём запустится, — но для
**закрытия фазы** эта строка на таком образце ничего не решает: её результат
не даёт ни `PASS`, ни `FAIL`. Возьмите образец с обычным печатным текстом.
...
Этот раздел — **архив**. Он не переписывается позднейшими уточнениями и не даёт
закрытия фазы. Текущий, закрывающий прогон — в следующем разделе.

### Прогон 2026-09-17 — первый прогон по исходному критерию IMG-E, 4 из 5

- **Дата:** 2026-09-17.
- **Базовая версия:** `a4c911c3691ba418e84a087e586fed79fc9333f7` — слитый
  `main`, в котором это руководство появилось впервые (PR #23).
- **Характер:** настоящая приёмка владельца по действовавшему тогда
  руководству. Ручной прогон на локальной машине, не CI и не автоматика.
- **Результат:**

| Сценарий | Статус владельца |
| --- | --- |
| IMG-A | PASS |
| IMG-B | PASS |
| IMG-C | PASS |
...
- **Итог: 4 из 5.** Macro Phase 4 **не закрылась** и осталась **ОТКРЫТА**.

**Что этот прогон показал.** Дело не в том, что изображение оказалось за
пределами того, что продукт принимает: PNG и JPEG поддерживаются, такое
изображение принимается и хранится как обычно, и сборка с `--image-ocr`
распознавание на нём запускает. Дело в **критерии закрытия**.

Исходная формулировка IMG-E делала условием закрытия приемлемое качество
распознавания практически на любом читаемом человеком изображении владельца.
Это шире того качества, которое можно честно **требовать** от фиксированной
политики Phase 4B — `eng+rus`, OEM 1, PSM 3, без коррекции ориентации, без
deskew, без повторов, без режима рукописи, без модели сценового текста, без
модели зрения и без классификатора типографики. Иначе говоря, критерий приёмки
трактовал качество распознавания на слишком широком классе изображений с
текстом как обязательную гарантию Macro Phase 4.

Поэтому обязательный образец IMG-E **сужен до обычного печатного текста** перед
новым, закрывающим прогоном. Что продукт принимает и на чём пытается
распознавать, при этом не меняется — меняется только то, на чём закрытие фазы
обязано показать приемлемый результат.

**Что этот прогон НЕ означает:**

- он **не** говорит, что то изображение было негодным;
- он **не** говорит, что Tesseract неисправен;
- он **не** говорит, что распознавание изображений в целом не работает;
- он **не** отменяется и **не** переписывается новым критерием: `FAIL` в строке
  IMG-E остаётся зафиксированным;
- и он **не** годится для закрытия фазы: закрытие требует прогона по
  **исправленному** критерию.

**Приватность.** Об использованном изображении здесь не записано ничего: ни
имени файла, ни пути, ни дайджеста, ни распознанного текста, ни исходного
текста, ни языка, ни размеров, ни описания содержимого. В этом архиве оно —
**одно настоящее изображение владельца**, и ничего больше.

---

## Результат закрывающей приёмки

**Закрывающий прогон выполнен 2026-09-18 и дал 5 из 5.** Это отдельный,
полностью новый прогон по **исправленному** критерию IMG-E; прогон 2026-09-17
остался в [истории](#история-прогонов-владельца) как есть и в этот счёт не
входит.

- **Дата:** 2026-09-18.
- **Базовая версия:** `f436492ce59cecbbce4ebed321ad7e4cf6326be8` — слитый
  `main`, содержащий исправленный критерий IMG-E.
- **Характер:** ручной прогон владельца за клавиатурой на локальной машине. Не
  CI, не облачная сессия, не автоматика. Один свежий каталог прогона, все пять
  сценариев выполнены заново внутри него.
- **Окружение:** Manjaro Linux; Python 3.13.13; системный Tesseract 5.5.3 с
  языковыми данными `eng` **и** `rus`.
- **Итог: 5 из 5 — `PASS`.**
- **Набор `[ocr]`:** чек-лист не требовал и не устанавливал его.
- **Набор `[ocr]` на машине владельца:** `PIL` и `pypdfium2` на машине
  присутствовали. Это наблюдение из шага 0; на статусы строк оно не влияет и
  приёмку не обесценивает. Но означает оно ровно одно: **этот ручной прогон сам
  по себе не доказывает, что распознавание изображений работает без них.** Эту
  границу зависимостей доказывает отдельная задача CI `Local image OCR`, которая
  удаляет `pypdfium2` и `Pillow` и проверяет через `importlib`, что их нет.
- **Приватность:** об изображении владельца здесь не записано ничего, кроме
  того, что это было **одно настоящее изображение владельца с обычным печатным
  текстом**.

> **Доказательства CI и доказательства владельца раздельны.** Автоматические
> наборы покрывают потоки, контракты, отказы и настоящий движок распознавания;
> изображение владельца они не отправляли и не оценивали. Оценку по IMG-E дал
> человек, глядя на свою картинку.

## Таблица результатов

Это таблица **закрывающего** прогона 2026-09-18. Все пять строк получены в
**одном и том же** прогоне. Результаты прогона 2026-09-17 сюда **не перенесены**
— ни одной строкой, включая четыре его `PASS`.
...
| --- | --- | --- | --- |
| IMG-A | PASS | обе подпроверки прошли: PNG и JPEG; жизненный цикл capture завершился успешно; процессор `image@0.1`; сегментов ноль; ключа `metadata.image_ocr` нет; сохранённые оригиналы побайтово те же | обычный режим, без ключей распознавания |
| IMG-B | PASS | настоящий локальный Tesseract отработал через `image-ocr@0.1`; ровно один `OCR`-сегмент; provenance, `position` и `spatial` — как описано в руководстве; `engine_invoked=true`; фиксированная политика `eng+rus` подтверждена; структурная `metadata.image` совпала с обычным режимом на тех же байтах; оригинал побайтово тот же; контрольное слово владелец увидел в распознанном | распознавание добавило сегмент и больше ничего |
| IMG-C | PASS | штатный предел в 20 000 000 закодированных пикселей сработал на настоящем сервере; capture всё равно завершился успешно; сегментов ноль; `engine_invoked=false`; `skipped_reason=encoded_pixel_limit`; `max_encoded_pixels=20000000`; ни `engine`, ни `engine_version`, ни `settings` для незапускавшегося движка не записаны; оригинал побайтово тот же | не `422`, не `503`, не `partial` |
| IMG-D | PASS | выполнен настоящий перезапуск процесса на том же каталоге данных; последний сервер запущен **без** `--image-ocr`; все пять `CaptureRecord` и `ContentObject` читались; снимки до и после совпали; все сохранённые оригиналы побайтово те же; сохранённый OCR-результат IMG-B читался без изменений; `restart_check.py` — общий `PASS` | долговечность через границу процесса |
| IMG-E | PASS | одно настоящее изображение владельца с обычным печатным текстом, отвечающее исправленной области действия; capture `201`; запись `complete`; content `200`; `engine_invoked=true`; ровно один сегмент; `type=ocr`; `source_type=ocr`; `position=0`; `spatial` пуст; длина распознанного текста больше нуля; оригинал побайтово тот же; владелец признал распознанное приемлемым по сравнению с тем, что читает на изображении | суждение человека; приватное содержимое не записано |
...
> **Macro Phase 4 ЗАКРЫТА** — этой таблицей, прогоном 2026-09-18, 5 из 5.
> Закрытие фиксирует историю проверки и **не добавляет ни одной возможности**.

---

## Что требовалось для закрытия Macro Phase 4 — и как это выполнено

**Все двенадцать условий выполнены прогоном 2026-09-18.** Список ниже не
удаляется: он остаётся долговечным объяснением того, **почему** закрытие
правомерно, а не просто записью о том, что оно состоялось.

Репозиторий мог правдиво сказать «Macro Phase 4 закрыта» только когда выполнено
**всё** перечисленное:

1. **IMG-A, IMG-B, IMG-C, IMG-D и IMG-E — все пять `PASS` в одном и том же
   прогоне.** Ни одного `NOT_RUN`, ни одного `FAIL`, ни одного `BLOCKED`.
   `BLOCKED` — не разновидность прохождения: отсутствие Tesseract, `eng` или
   `rus` закрытие останавливает. Строки из разных прогонов не складываются, и
   `PASS` из прогона 2026-09-17 в этот счёт **не входят**.
   IMG-E засчитывается только на образце в области действия — обычный печатный
   текст.
...
3. Записан **точный SHA слитого `main`**, на котором прогон выполнялся, и этот
   `main` содержит настоящее руководство **вместе с исправленным критерием
   IMG-E**. Прогон на более раннем `main` для закрытия не годится.
...
**Как это выполнено.** Пять `PASS` получены в одном прогоне 2026-09-18 (1);
прогон был ручным, человеком за клавиатурой на Manjaro Linux (2); базовая
версия — `f436492ce59cecbbce4ebed321ad7e4cf6326be8`, слитый `main` с
исправленным критерием IMG-E (3); доказательства CI и владельца записаны
раздельно, и ничто не выдаёт одно за другое (4); использовано одно настоящее
изображение владельца с обычным печатным текстом, и ничего больше о нём не
записано (5); чек-лист не требовал и не устанавливал `[ocr]`, а присутствие
`PIL` и `pypdfium2` на машине отмечено как наблюдение, которое на статусы не
влияет и границу зависимостей не доказывает (6); закрытие не добавило ни одной
возможности (7); `SCHEMA_VERSION` остался `0.2` (8); семантика Phase 4A и
Phase 4B не пересмотрена (9); ADR-019 и ADR-020 не тронуты (10); закрывающее
изменение — только документация (11); проверки качества на нём проходят (12).
...
Phase 4C — **одна** макро-фаза, и все её изменения — только документация. Её
фактический ход:

1. **PR #23** добавил это руководство и одну ссылку на него из README. Статус
   фазы он не менял.
2. Владелец прошёл руководство руками на слитом `main`
   `a4c911c3691ba418e84a087e586fed79fc9333f7` и получил **4 из 5**: IMG-A…IMG-D
   `PASS`, IMG-E `FAIL`. Подробности — в
   [истории прогонов](#история-прогонов-владельца).
3. **PR #24** — уточнение области действия приёмки после того прогона. Оно
   сузило IMG-E до обычного печатного текста, внесло прогон 2026-09-17 в архив
   как состоявшуюся приёмку и не тронуло ни продукт, ни `docs/ARCHITECTURE.md`,
   ни ADR. Macro Phase 4 после него оставалась **ОТКРЫТА**.
4. Владелец провёл **новый полный прогон** 2026-09-18 против слитого `main`
   `f436492ce59cecbbce4ebed321ad7e4cf6326be8`, содержащего это уточнение, и
   получил **5 из 5**.
5. **Закрывающее изменение** — тоже только документация — записало этот
   результат здесь и закрыло Macro Phase 4 в `docs/ARCHITECTURE.md`.

Шаг 3 не изобретал новую фазу: это было уточнение критерия приёмки внутри
Phase 4C, а не ещё один срез реализации. **Никакой Phase 4D нет и не
предполагается.**
...
- Он не обещает **качества** распознавания на рукописи, начертаниях «под
  рукопись», каллиграфии, логотипах, декоративных и сильно стилизованных
  начертаниях, изогнутом или деформированном тексте, тексте в сильной
  перспективе, сильно перекрытом тексте, тексте с очень низким контрастом и на
  задачах понимания изображения. Это ограничение **требования к закрытию**, а не
  границы поддержанного входа: такие PNG и JPEG UniMem принимает и хранит как
  обычно, и сборка с `--image-ocr` запускает на них движок как на любом другом
  поддержанном изображении. Обязательный образец IMG-E просто не проверяет это
  качество, и закрытие фазы его не обещает.
