...
- Статусы в [таблице результатов](#таблица-результатов) ставит **только
  владелец**. Сейчас там записан завершённый прогон от 2026-09-14; новый полный
  прогон начинает свою таблицу заново с `NOT_RUN`. Пройденный синтетический
  контроль не означает, что прошёл ваш собственный документ.
...
- **Дата:** 2026-09-14.
- **Базовая версия:** `d665bf57f4debb9c7a6506b8dd279d9d838da90d` — смёрженный
  `main` (руководство пришло в PR #19).
- **Окружение:** локальная машина владельца, Manjaro Linux; Python 3.13.13;
  системный Tesseract с языковыми данными `eng` **и** `rus` — движок сообщил
  версию 5.5.3 при осмотре OCR-содержимого; `pypdfium2` 5.13.0 по метаданным
  OCR.
- **Итог: 9 из 9 — `PASS`.**
- Это **реальный локальный прогон владельца, выполненный вручную.** Не CI, не
  облачная сессия и не автоматика: шаги выполнял человек на своей машине.
- Использовались синтетические контроли из фикстур репозитория **и** один
  реальный PDF, предоставленный владельцем.
- Приватное содержимое документа владельца здесь не записано: ни имя файла, ни
  путь, ни дайджест, ни извлечённый текст.

## Таблица результатов

Таблица заполнена владельцем по итогам прогона 2026-09-14. Допустимые значения:
`NOT_RUN`, `PASS`, `FAIL`, `BLOCKED`; новый полный прогон начинает свою таблицу
заново с `NOT_RUN`.

| Сценарий | Статус владельца | Доказательство | Заметки |
| --- | --- | --- | --- |
| Шаг 4 — контроли без текстового слоя | PASS | `embedded_text.py`: у английского и русского сканов текста нет; у `mixed.pdf` текст только на стр. 1; у `text.pdf` — на стр. 1 и 3 | контроли годны для DOC-C и DOC-D |
| DOC-A — обычный PDF, обычный режим | PASS | upload 200, capture 201, record 200, content 200; долговечный `COMPLETE`; тип `document`; заголовок из метаданных PDF; сегменты `TEXT`/`ORIGINAL`; физические стр. 1 и 3; позиции 0 и 1; `pdf@0.1`; байты оригинала совпали | разрыв в страницах, непрерывность позиций |
| DOC-B — DOCX, обычный режим | PASS | `COMPLETE`; 4 сегмента в порядке тела: абзац, строка таблицы, строка таблицы, абзац; строки разделены табуляцией; номеров страниц нет; `docx@0.1`; `ORIGINAL`; заголовок взят из свойств документа; контроль без заголовка дал `title=None`; присланный заголовок победил заголовок документа; байты оригинала совпали | |
| DOC-C — скан при выключенном OCR | PASS | upload 200; capture `422 processing_failed`; долговечный `FAILED`; content `404 not_found`; пустой `ContentObject` не создан; staged-оригинал остался и побайтово тот же | ожидаемый отказ, не дефект |
| DOC-D — тот же скан при включённом OCR (eng + rus) | PASS | тот же staged скан под **новым** capture id: 201, `COMPLETE`; сегмент `OCR`/`OCR` на физической стр. 1; `pdf-ocr@0.1`; ожидаемая английская фраза распознана; русский контроль — 201 и content 200, русская фраза распознана верно; `metadata.pdf_ocr` зафиксировала `eng+rus`, Tesseract, `pypdfium2` и настройки | capture из DOC-C остался `FAILED` с content 404 — включение OCR его не оживило |
| DOC-E — смешанный PDF | PASS | capture 201, content 200; стр. 1 осталась `TEXT`/`ORIGINAL`; стр. 2 стала `OCR`/`OCR`; стр. 3 не дала сегмента; позиции непрерывны 0 и 1; `page_count=3`, `embedded_text_pages=[1]`, `ocr_attempted_pages=[2,3]`, `ocr_pages_without_text=[3]` | пустой результат распознавания — наблюдение, не вывод о пустоте страницы |
| DOC-F — перезапуск процесса и обратное чтение | PASS | до перезапуска: PDF 200/200, DOCX 200/200, упавший скан 200/404, OCR-скан 200/200, смешанный 200/200; OCR-сервер остановлен и запущен заново на **том же** каталоге данных **без** `--pdf-ocr`; после перезапуска все HTTP-состояния совпали; разобранные снимки `CaptureRecord` и `ContentObject` прошли проверку долговечности; байты оригиналов совпали у обычного PDF, DOCX, OCR-PDF и смешанного PDF | чтение OCR-содержимого не потребовало движка |
| DOC-G — идентичность и безопасные отказы | PASS | тот же capture id повторно — `409 capture_already_exists`; те же байты под новым id — новые content id и asset id при том же оригинальном SHA-256 и той же raw-идентичности; правдоподобная, но не положенная ссылка — `422 capture_material_unavailable`, записи нет, последующий GET 404; путь `/etc/passwd` в роли `file_ref` — `422 unsupported_payload`, записи нет, GET 404 | ни путь, ни URL не были приняты за staged-материал |
| Свои документы | PASS | один реальный PDF, предоставленный владельцем: upload 200, capture 201, record 200, content 200; долговечный `COMPLETE`; байты оригинала совпали; владелец глазами подтвердил, что ожидаемый текст присутствует и соответствие страницам верное | приватное содержимое не записано: ни имя файла, ни путь, ни дайджест, ни текст |

**Macro Phase 3 ЗАКРЫТА приёмкой владельца 2026-09-14** — *Macro Phase 3 is
CLOSED by owner acceptance on 2026-09-14.*

Закрытие фиксирует историю проверки и **не добавляет ни одной возможности**: ни
формата, ни маршрута, ни поля контракта, ни изменения семантики. Реализация PDF,
DOCX и необязательного локального OCR осталась такой же, какой была на
`d665bf5`. Фазы 0, 1 и 2 закрытия этой фазы не касаются и остаются закрытыми.

---

## Что уже проверено, а что нет

> **Исторический раздел — про облачную сессию, в которой это руководство
> писалось.** Он относится к коммиту `00bcb67` и **не** является приёмкой
> владельца: её результат записан выше, в
> [Результате приёмки владельцем](#результат-приёмки-владельцем). Оба
> утверждения верны и относятся к разным прогонам: в облачном окружении
> системного Tesseract не было, на машине владельца он был.
